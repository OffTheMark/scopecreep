...
from .forms import LoginForm, SignupForm, TopicForm, SuggestionForm, CommentForm, ChangePasswordForm
...
class ProfileView(LoginRequiredMixin, generic.FormView):
    ...
    def get_context_data(self, **kwargs):
        data = super().get_context_data(**kwargs)
        data["topics"] = Topic.objects.filter(submitter=self.request.user)
        return data
